...
export const initConsumerClientLogic = (logger: LoggerService): Kafka | undefined => {
  if (!process.env.KAFKA_BROKER || 
    ...
    logger.warn('Kafka environment variables are missing or invalid. Kafka consumer will be disabled.');
    return undefined;
  }

  const brokers = parseBrokersLogic(process.env.KAFKA_BROKER);
  if (brokers.length === 0) {
    logger.warn('Kafka brokers list is empty. Kafka consumer will be disabled.');
    return undefined;
  }
